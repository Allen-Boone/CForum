...
export default {
	async fetch(
		...
		const ensureSchema = async () => {
			await db
				.prepare('ALTER TABLE posts ADD COLUMN badge TEXT')
				.run()
				.catch(() => {});

			await db
				.prepare(
					'ALTER TABLE posts ADD COLUMN reward_points INTEGER DEFAULT 0'
				)
				.run()
				.catch(() => {});
			...
			await db
				.prepare('ALTER TABLE users ADD COLUMN reg_ip TEXT')
				.run()
				.catch(() => {});
			...
			await db
				.prepare(`
					CREATE TABLE IF NOT EXISTS site_badges (
						...
						description TEXT,
						color TEXT DEFAULT 'border-amber-500 bg-amber-500/10 text-amber-300',
						created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
					)
				`)
				.run();
			...
			await db
				.prepare(
					'CREATE INDEX IF NOT EXISTS idx_users_reg_ip ON users(reg_ip)'
				)
				.run()
				.catch(() => {});
		};
	...
	<url>
		...
		if (
			...
			try {
				...
				const storedCode = await hashText(
					`${email}:${code}`
				);
      ...
      <td align="center">
              ...
              <p style="margin:0;font-size:11px;color:#8c959f;text-align:center;">This is an automated notification from CForum. Please do not reply directly.</p>
				...
				await db
					.prepare(`
						...
						VALUES (?, ?, ?)
					`)
					.bind(email, storedCode, expiresAt)
					.run();
		...
		if (
			...
			try {
				...
				const result = await db
					.prepare(`
						SELECT id, user_id, username, reason,
						       duration, created_at
						FROM blackhouse
						ORDER BY id DESC
						LIMIT 50
		...
		if (url.pathname === '/api/me' && method === 'GET') {
			try {
				...
				const user = await db
					.prepare(`
						SELECT id, username, email, role,
						       avatar_url, points, title,
						       badges, last_checkin_date,
						       created_at
						FROM users
						WHERE id = ?
					`)
				...
				const [postsCount, commentsCount] =
					await Promise.all([
						db
							.prepare(`
								SELECT COUNT(*) AS count
				...
				return jsonResponse({
					...
					stats: {
						posts_count: postTotal,
						comments_count: commentTotal,
						points
					}
				});
			} catch (error) {
		...
		if (
			...
			try {
				...
				if (body.username !== undefined) {
					const newName = normalizeUsername(
						body.username
					);
				...
				if (body.title !== undefined) {
					...
					if (requestedTitle.length > 32) {
						return jsonResponse(
							{ error: '称号长度不能超过 32 个字符' },
							400
						);
					}
					...
					if (
						currentUser.role !== 'admin' &&
						(requestedTitle.includes('站长') ||
							requestedTitle.includes('管理员') ||
							requestedTitle.includes('官方') ||
							requestedTitle
								.toLowerCase()
								.includes('admin'))
					) {
		...
		if (
			...
			try {
				...
				const anonymousEmail =
					`deleted_${currentUser.id}_${Date.now()}@freedom.invalid`;
		...
		if (
			...
			try {
				...
				const submittedCodeHash = await hashText(
					`${email}:${code}`
				);
		...
		if (
			...
			try {
				...
				const categories = await db
					.prepare(`
						SELECT id, name,
						       COALESCE(NULLIF(sort_order, 0), id) AS sort_order,
						       created_at
						FROM categories
						ORDER BY
							COALESCE(NULLIF(sort_order, 0), id) ASC,
							id ASC
					`)
					.all();
		...
		if (
			/^\/api\/admin\/categories\/\d+\/sort$/.test(
				url.pathname
			) &&
			method === 'POST'
		) {
			...
			try {
				...
				let query = `
					SELECT
						...
						p.badge,
						COALESCE(p.reward_points, 0) AS reward_points,
				...
				if (currentUser.role !== 'admin') {
					...
					if (latestPost?.created_at) {
						const lastTime = new Date(
							latestPost.created_at.endsWith(
								'Z'
							)
								? latestPost.created_at
								: `${latestPost.created_at}Z`
						).getTime();
		...
		if (
			...
			try {
				...
				const updated = await db
					...
					`)
					.bind(postId)
					.first<{ reward_points: number }>();

				return jsonResponse({
					success: true,
					total_reward: Number(
						updated?.reward_points || 0
					)
				});
			} catch (error) {
				return handleError(error);
